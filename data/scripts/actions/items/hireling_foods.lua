...
function hirelingFoods.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif dish.healing then
		player:addHealth(math.floor(player:getMaxHealth() * 0.3))
	elseif dish.manaRestore then
		player:addMana(math.floor(player:getMaxMana() * 0.3))
	end

	player:say(dish.message, TALKTYPE_MONSTER_SAY)
	player:setExhaustion("special-foods-cooldown", 10 * 60 * 1000)
	item:remove(1)
	return true
end

for itemId in pairs(gourmetDishes) do
	hirelingFoods:id(itemId)
end
